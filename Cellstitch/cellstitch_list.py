...
def run_cellstitch(input_path, output_path, batch_xy, batch_yz, batch_xz):
    ...
    eval_kwargs = {
        "channel_axis": 0, # Matches your working script
        "diameter": 109.13,
    ...
    # 2. Segment YZ (Transpose Z, C, Y, X -> Y, C, Z, X)
    print(f"Segmenting YZ (batch_size={batch_yz})...")
    img_yz = img.transpose(2, 1, 0, 3)
    ...
    # 3. Segment XZ (Transpose Z, C, Y, X -> X, C, Y, Z)
    print(f"Segmenting XZ (batch_size={batch_xz})...")
    img_xz = img.transpose(3, 1, 2, 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("input", help="Path to input 4D TIFF (Z, C, Y, X)")
    parser.add_argument("output", help="Path to output directory")
    parser.add_argument("--batch_xy", type=int, default=32)
    parser.add_argument("--batch_yz", type=int, default=32)
    parser.add_argument("--batch_xz", type=int, default=32)
    args = parser.parse_args()

    run_cellstitch(Path(args.input), Path(args.output), args.batch_xy, args.batch_yz, args.batch_xz)
