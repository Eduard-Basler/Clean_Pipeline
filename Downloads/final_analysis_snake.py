...
# ==========================================
# 1. SNAKEMAKE DIRECTORY & IO CONFIGURATION
# ==========================================
# Instead of searching with glob, Snakemake hands the exact files directly to the script
raw_path       = snakemake.input.raw_full
cell_mask_path = snakemake.input.cell_mask
nuc_mask_path  = snakemake.input.nuc_mask

# getattr safely handles if 'bact_mask' is missing (for non-sATA samples)
bact_mask_path = getattr(snakemake.input, "bact_mask", None)

cell_out       = snakemake.output.cell_stats
bact_out       = getattr(snakemake.output, "bact_stats", None)

VAL_DIR  = snakemake.params.val_dir
...
# ==========================================
# 2. FILE MATCHING & LOAD DATA
# ==========================================
# Snakemake runs one file per job, so we define base_name from wildcards instead of a loop
sample_name = snakemake.wildcards.sample_name
series      = snakemake.wildcards.series
base_name   = f"{sample_name}_s{series}"
...
if raw_stack.ndim == 4:
    if raw_stack.shape[1] < 10:  # Format: (Z, C, Y, X)
        ...
        # Move channels to the very end for regionprops
        ...
        # Channels are already at the end, just assign it
        raw_multi = raw_stack 
    ...
    # Use raise instead of continue so Snakemake knows this specific job failed
    raise ValueError(f"⚠️ ERROR: Raw image is not 4D (Shape: {raw_stack.shape}).")

cell_mask = imread(cell_mask_path)
nuc_mask  = imread(nuc_mask_path)

# Load bacteria mask if provided by Snakemake, otherwise create an empty array to prevent crashes
if bact_mask_path and os.path.exists(bact_mask_path):
    bact_mask = imread(bact_mask_path)
else:
    bact_mask = np.zeros_like(cell_mask)

# Failsafe check to ensure shapes actually match
if nuc_mask.shape[:3] != nuc_raw.shape[:3]: 
    raise ValueError(f"⚠️ ERROR: Shape mismatch! Mask is {nuc_mask.shape}, Raw is {nuc_raw.shape}.")
...
# Only save bacteria stats if the file was expected in Snakemake's output
if bact_out and len(df_bact) > 0:
    df_bact.to_csv(bact_out, index=False)
elif bact_out:
    # Touch an empty file if Snakemake requires it but there were no bacteria found
    pd.DataFrame().to_csv(bact_out, index=False)
